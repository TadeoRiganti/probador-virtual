...
BRAINSTORMING:
a) A QR code to the same page, maybe with the #id of the 3D model.
  ✔️ Easy to implement. No extra development needed.️
  ❌ It's not straight-foward, or at least in the best case, users needs to figure it out and look for the Ar button.
...
b) To have a dedicated page for each model, that when visiting that URL allows to download the model and then prompting the user to view it int it's space.
  ✔️ It can improve user experience by adding general tips to help new users use this feature.
  ❌ It will be needed to code an extra page for each model.
      💡 Since 3D assets are not embedded on each page, it's possible to resolve this with a dynamic page wich loads the model's id);
